...
describe('friendly generation errors', () => {
  it('turns missing provider configuration into a user-safe message', () => {
    expect(getFriendlyGenerationError(new Error('RUNWARE_API_KEY is not configured.')))
      .toBe('Generation provider is not configured. Check the environment settings and try again.');
  });
