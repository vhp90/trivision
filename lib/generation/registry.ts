...
export const generationModels: GenerationModelDefinition[] = [
  ...
  },
  {
    ...
    description: 'Self-hosted TRELLIS.2 on Lightning AI with queued GLB generation.',
    availability: 'enabled',
    capabilities: {
      inputKinds: ['image'],
      promptSupport: 'none',
    ...
    defaultOutputFormat: 'glb',
    primaryInputLabel: 'Reference Image',
    primaryInputDescription: 'Upload a single source image. Use RMBG first when the image needs a transparent background.',
    ...
    parameters: [
      ...
      parameter({
        ...
        description: 'Run the server preprocessing step before generation.',
        section: 'Generation',
        type: 'boolean',
        defaultValue: true,
      }),
      parameter({
        key: 'decimationTarget',
        label: 'Decimation Target',
      ...
      parameter({
        key: 'simplifyLimit',
        label: 'Simplify Limit',
        description: 'Maximum face count before simplification.',
        section: 'Mesh Output',
        type: 'number',
